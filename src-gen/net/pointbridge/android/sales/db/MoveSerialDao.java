...
/** 
 * DAO for table MOVE_SERIAL.
*/
public class MoveSerialDao extends AbstractDao<MoveSerial, String> {

    public static final String TABLENAME = "MOVE_SERIAL";

    /**
     * Properties of entity MoveSerial.<br/>
     * Can be used for QueryBuilder and for referencing column names.
    */
    public static class Properties {
        public final static Property Ms_id = new Property(0, String.class, "ms_id", true, "MS_ID");
        public final static Property Md_id = new Property(1, String.class, "md_id", false, "MD_ID");
        public final static Property Mm_id = new Property(2, String.class, "mm_id", false, "MM_ID");
    ...
    private Query<MoveSerial> moveMaster_Mv_serial_fk1Query;

    public MoveSerialDao(DaoConfig config) {
        super(config);
    }
    
    public MoveSerialDao(DaoConfig config, DaoSession daoSession) {
    ...
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        ...
        db.execSQL("CREATE TABLE " + constraint + "'MOVE_SERIAL' (" + //
                "'MS_ID' TEXT PRIMARY KEY ASC NOT NULL ," + // 0: ms_id
                "'MD_ID' TEXT," + // 1: md_id
                "'MM_ID' TEXT," + // 2: mm_id
        ...
        // Add Indexes
        db.execSQL("CREATE INDEX " + constraint + "IDX_MOVE_SERIAL_MM_ID ON MOVE_SERIAL" +
                " (MM_ID);");
    }

    /** Drops the underlying database table. */
    public static void dropTable(SQLiteDatabase db, boolean ifExists) {
        String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "'MOVE_SERIAL'";
        db.execSQL(sql);
    }

    /** @inheritdoc */
    @Override
    protected void bindValues(SQLiteStatement stmt, MoveSerial entity) {
        stmt.clearBindings();
 
        String ms_id = entity.getMs_id();
        if (ms_id != null) {
            stmt.bindString(1, ms_id);
        }
 
        String md_id = entity.getMd_id();
        if (md_id != null) {
            stmt.bindString(2, md_id);
        }
 
        String mm_id = entity.getMm_id();
        if (mm_id != null) {
            stmt.bindString(3, mm_id);
        }
    ...
    /** @inheritdoc */
    @Override
    public MoveSerial readEntity(Cursor cursor, int offset) {
        MoveSerial entity = new MoveSerial( //
            cursor.isNull(offset + 0) ? null : cursor.getString(offset + 0), // ms_id
            cursor.isNull(offset + 1) ? null : cursor.getString(offset + 1), // md_id
            cursor.isNull(offset + 2) ? null : cursor.getString(offset + 2), // mm_id
    ...
    /** @inheritdoc */
    @Override
    public void readEntity(Cursor cursor, MoveSerial entity, int offset) {
        entity.setMs_id(cursor.isNull(offset + 0) ? null : cursor.getString(offset + 0));
        entity.setMd_id(cursor.isNull(offset + 1) ? null : cursor.getString(offset + 1));
        entity.setMm_id(cursor.isNull(offset + 2) ? null : cursor.getString(offset + 2));
    ...
    /** @inheritdoc */
    @Override
    protected String updateKeyAfterInsert(MoveSerial entity, long rowId) {
        return entity.getMs_id();
    }
    
    /** @inheritdoc */
    @Override
    public String getKey(MoveSerial entity) {
        if(entity != null) {
            return entity.getMs_id();
        } else {
            return null;
    ...
    /** Internal query to resolve the "mv_serial_fk1" to-many relationship of MoveMaster. */
    public List<MoveSerial> _queryMoveMaster_Mv_serial_fk1(String mm_id) {
        synchronized (this) {
            if (moveMaster_Mv_serial_fk1Query == null) {
                QueryBuilder<MoveSerial> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.Mm_id.eq(null));
                queryBuilder.orderRaw("MS_ID ASC");
                moveMaster_Mv_serial_fk1Query = queryBuilder.build();
            }
        }
        Query<MoveSerial> query = moveMaster_Mv_serial_fk1Query.forCurrentThread();
