...
public class BackgroundServicePluginLogic {
	...
	public static final String ACTION_ASK_JOB = "askJob";
	...
	public boolean isActionValid(String action) {
		...
		if(ACTION_ASK_JOB.equals(action)) result = true;
			...
			if ((data != null) &&
				...
				if (result == null) {
					...
					if (service.isServiceRunning()) {
						...
						if (ACTION_ASK_JOB.equals(action)) result = service.askJob(data);
	...
	protected class ServiceDetails {
		...
		public ExecuteResult startService()
		{
			...
			try {
				...
				if (this.bindToService()) {
					this.mApi.addListener((BackgroundServiceListener)mContext);
		...
		public ExecuteResult askJob(JSONArray data)
		{
			ExecuteResult result = null;
			...
			try {
				if (this.isServiceRunning()) {
					...
					try {
						obj = data.get(1);
						String strdata=mApi.askJob(obj.toString());
						//System.out.println("askJob :"+strdata);
						result = new ExecuteResult(ExecuteStatus.OK, createJSONDataResult(true, ERROR_NONE_CODE, ERROR_NONE_MSG,strdata));
					} catch (JSONException e) {
						result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, e.getMessage()));
					}
				} else {
		...
		private JSONObject createJSONDataResult(Boolean success, int errorCode, String errorMessage,String adata) {
